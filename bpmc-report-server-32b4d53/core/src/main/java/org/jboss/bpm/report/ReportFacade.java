...
public class ReportFacade
{
  ...
  public ReportFacade()
  {
    ...
    catch (BirtInitException e)
    {
      ...
      log.info("Jasper service has not been activated. Please check the DEBUG log for further details.");
      ...
      throws BirtInitException
  {
    if(!isInitialized)
    {
      ...
      log.info("JASPER home: " +iConfig.getBirtHome());
      log.info("Output dir: " +iConfig.getOutputDir());
      log.info("Report dir: " +iConfig.getReportDir());

      try
      {
        this.jasperService = new JasperService(iConfig);
        this.jasperService.createAsync();
      }
      catch (Throwable t)
      {
  ...
  private File resolveBirtDataDir()
  {
    ...
    try
    {
      ...
      serverDataDir = new File(jmxConfig.getServerDataDir(), "jasper");
    }
    catch (Exception e)
    {
      ...
      if(System.getProperty("catalina.home")!=null)
        serverDataDir = new File(System.getProperty("catalina.home") + "/jasper");
      else
  ...
  )
  {
    ...
    try
    {
	  log.info("Filename from viewReportHtml :"+fileName);
  ...
  {
	log.info("filename from renderReportHtml " + fileName);
    assertBirtAvailability();
  ...
  public Response getReportConfig()
  {
    ...
    List<ReportReference> refs = jasperService.getReportReferences();
